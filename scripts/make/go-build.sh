...
# Allow overriding the go command from environment.  For example, to build two
# releases with two different Go versions and test the difference.
go="${GO:-go}"
readonly go
...
# and are not empty.
version_pkg="scr/internal/version"
...
# Allow limiting the build's parallelism.
...
# Allow specifying a different output name.
out="${OUT:-srv}"
readonly out
...
# Allow enabling the race detector.  Unfortunately, that means that cgo must be
# enabled.
if [ "${RACE:-0}" -eq '0' ]
then
	cgo_enabled='0'
...
"$go" build --ldflags "$ldflags" "$race_flags" --trimpath "$o_flags" "$v_flags" "$x_flags" "$app"